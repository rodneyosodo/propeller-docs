...
| `k8s.logLevel`                           | `debug` \                                        | `info` \    | `warn` \  | `error`                                                                                         |
...
| `connectionConfig.clientId`              | string                                           | Yes         | —         | Atom client credential ID; must match `proplet_id` in alive messages for external proplets   |
| `connectionConfig.clientKey`             | string                                           | No*         | —         | Atom client secret (inline). Exactly one of `clientKey` or `clientKeySecretRef` must be set  |
