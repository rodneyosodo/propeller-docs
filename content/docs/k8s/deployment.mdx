...
The operator repo also ships a sample per [WASM example](/k8s/examples) — `propeller_v1_task_compute.yaml`, `_string_input.yaml`, `_http_server.yaml`, `_http_client.yaml`, `_attestation_test.yaml`, `_hal_test.yaml`, `_tee.yaml`, `_filesystem.yaml`, `_wasi_http.yaml` — plus `propeller_v1_proplet_full.yaml` (every optional `k8s.env.*` flag enabled, for the feature-gated ones) and `propeller_v1_proplet_wasi_nn.yaml` (dedicated WASI-NN image). See [Running Other WASM Examples](/k8s/examples) for what each needs and the verified output.

`propeller_v1_task_with_image.yaml` and `spec.imageUrl` generally need the registry proxy service running — `propeller_proxy_deployment.yaml` is a plain Deployment + Service for it (not a CRD; see [Running Other WASM Examples: Deploying the registry proxy](/k8s/examples#deploying-the-registry-proxy)).
