...
- TPM2 TSS development headers (`libtss2-dev libssl-dev pkg-config`, required for `make proplet` on x86_64 Linux — see [Proplet TEE Attestation Dependencies](#proplet-tee-attestation-dependencies))
...
##### Proplet TEE Attestation Dependencies

Building the proplet for **x86_64 Linux** requires the TPM2 TSS development headers, because the `amd-sev` feature pulls in `az-snp-vtpm` → `tss-esapi-sys`, whose build script probes for `tss2-sys.pc`:

```bash
sudo apt-get install -y libtss2-dev libssl-dev pkg-config
```

Without these headers the build fails in `tss-esapi-sys` with `Failed to find tss2-sys library`.

Azure SEV-SNP attestation (`az-snp-vtpm-attester`, `elastic-tee-hal/amd-sev`) is enabled for **x86_64 only**, so per-architecture support differs:

| Target        | Builds                       | Azure SEV-SNP attestation                      |
| ------------- | ---------------------------- | ---------------------------------------------- |
| x86_64 Linux  | Yes (requires `libtss2-dev`) | Yes                                            |
| aarch64 Linux | Yes (no TSS headers needed)  | No — SEV-SNP/TDX are x86_64-only               |
| riscv64 Linux | Yes (no TSS headers needed)  | No — `tss-esapi-sys` rejects the riscv64 tuple |

This is intentional, not a gap: there is no SEV-SNP or TDX hardware on ARM or RISC-V, so there is nothing to attest with `az-snp-vtpm` there. If TPM-based attestation on riscv64 is ever needed for another purpose, that support would have to come from upstream [`tss-esapi-sys`](https://github.com/parallaxsecond/rust-tss-esapi), whose build script explicitly supports only x86_64/aarch64 Linux (plus Arm Linux and x86_64 macOS). Cross-compiling `tss-esapi-sys` for aarch64 additionally requires an aarch64 sysroot (`libtss2-dev:arm64` with `PKG_CONFIG_ALLOW_CROSS=1`), which is why the proplet build skips that dependency off x86_64 instead.
