...
Credentials are defined as a `defaults` struct in `src/main.c` and can be overridden at runtime through the Zephyr Settings subsystem (persisted in NVS under the `proplet` prefix). Edit the `defaults` struct in `src/main.c`:
...
static const struct proplet_credentials defaults = {
    ...
    .mqtt_broker_host = "10.42.0.1", /* Replace with your broker's IP */
...
`proplet_id` is the Propeller Entity ID used as the MQTT username; `api_key` is the Entity secret used as the MQTT password. When TLS is enabled, `tls_client_cert` and `tls_client_key` hold the mTLS client credentials.

At runtime, settings under the `proplet` prefix (e.g. `proplet/proplet_id`, `proplet/wifi_ssid`, `proplet/mqtt_broker_host`, `proplet/api_key`) override the compiled defaults — see `src/credentials.c`. The default MQTT broker host/port are also declared in `src/mqtt_client.h` (`MQTT_BROKER_HOSTNAME`, `MQTT_BROKER_PORT`, `MQTT_BROKER_PORT_TLS`).

The ESP32-S3 radio is **2.4 GHz only** — 5 GHz networks are not supported.
...
| `credentials.c`  | NVS-backed credential loading and runtime overrides   |
...
CONFIG_MQTT_LIB_TLS=y        # TLS enabled by default (port 8883)
CONFIG_MQTT_LIB_TLS_USE_ALPN=y

# HTTP client (used for registry/model fetching)
CONFIG_HTTP_CLIENT=y

# Settings (NVS-backed credential overrides)
CONFIG_SETTINGS=y
...
# Metrics (heap stats, thread monitoring)
CONFIG_SYS_HEAP_RUNTIME_STATS=y
CONFIG_THREAD_MONITOR=y
# CONFIG_CPU_LOAD is not supported on Xtensa targets
```

`CONFIG_MQTT_CLEAN_SESSION=n` preserves broker subscriptions across reconnections. `CONFIG_MQTT_KEEPALIVE=30` sets the keepalive to 30 seconds. `CONFIG_MQTT_LIB_TLS=y` enables MQTT over TLS by default, so the proplet connects on port `8883` unless TLS is compiled out. `CONFIG_WIFI_ESP32` is board-specific and belongs in the board overlay file, not here.

### Board-Specific Configuration

Board-specific overrides are provided in `boards/esp32s3_devkitc_procpu.conf`:
...
The Embedded Proplet connects to the broker using `proplet_id` as the MQTT **client ID** and **username**, and the `api_key` as the **password**. The credentials must match the Atom entity (kind `service`, profile `Service Account`) registered with the broker, otherwise the connection is rejected.

The default build enables MQTT over TLS (`CONFIG_MQTT_LIB_TLS=y` in `prj.conf`), so the proplet connects on port `8883` (`MQTT_BROKER_PORT_TLS`). For a plaintext local broker, either compile with TLS disabled or provide a CA certificate / mTLS client credentials. The proplet identifies itself to the Manager with the namespace `"embedded"` in all metrics payloads.

### MQTT Topics

| Direction | Topic                                                 | Purpose                              |
| --------- | ----------------------------------------------------- | ------------------------------------ |
| Publish   | `m/<tenant_id>/c/<channel_id>/control/proplet/create`       | Discovery on startup                 |
| Publish   | `m/<tenant_id>/c/<channel_id>/control/proplet/alive`        | Periodic alive heartbeat             |
| Publish   | `m/<tenant_id>/c/<channel_id>/control/proplet/metrics`      | System metrics (CPU, memory, uptime) |
| Publish   | `m/<tenant_id>/c/<channel_id>/control/proplet/task_metrics` | Per-task execution metrics           |
| Publish   | `m/<tenant_id>/c/<channel_id>/control/proplet/results`      | Task execution results               |
| Publish   | `m/<tenant_id>/c/<channel_id>/registry/proplet`             | Request a WASM module from registry  |
| Subscribe | `m/<tenant_id>/c/<channel_id>/control/manager/start`        | Receive task start command           |
| Subscribe | `m/<tenant_id>/c/<channel_id>/control/manager/stop`         | Receive task stop command            |
| Subscribe | `m/<tenant_id>/c/<channel_id>/registry/server`              | Receive WASM module from registry    |
...
```c
publish_discovery(tenant_id, proplet_id, channel_id, description, tags, location, version);
```

The payload includes `proplet_id`, `namespace`, and a `metadata` object with the description, tags, location, IP address, OS (`zephyr`), CPU architecture, total memory, `proplet_version`, and `wasm_runtime` (`wamr`). The Manager receives this and adds the Proplet to its fleet.
...
- **System metrics** (every 30s): CPU load and heap usage
...
| `MANAGER_COORDINATOR_URL`   | No       | `http://coordinator-http:8080` | FL coordinator service URL                                      |
...
| `FL_ROUND_ID`               | No       | —                              | Legacy FL round identifier (falls back to `ROUND_ID`)           |
| `FL_FORMAT`                 | No       | `f32-delta`                    | FL update wire format reported in the results envelope          |
| `FL_NUM_SAMPLES`            | No       | `1`                            | Number of local training samples reported in the update         |

#### FML Data Fetch Flow

1. **Model**: HTTP GET `<MODEL_REGISTRY_URL>/models/<version>` (the version is parsed from the last segment of `MODEL_URI`, e.g. `.../global_model_v3`). If HTTP fails, the proplet falls back to the `MODEL_URI` MQTT subscription and waits for model data to arrive over MQTT.
...
For FML tasks, results are published as an update to `fl/rounds/<round_id>/updates/<proplet_id>` instead of the standard results topic.
