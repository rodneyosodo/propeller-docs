...
### Enable TLS on the KBS endpoint

The Docker Compose setup above starts KBS with `insecure_http = true` in `kbs/config/docker-compose/kbs-config.toml`, so `http://localhost:8082` is unencrypted — fine for local testing, but not for anything that crosses a network boundary. KBS supports TLS termination natively via the `[http_server]` section; no reverse proxy is required.

Generate a self-signed certificate (replace `localhost` with your KBS hostname for a real deployment, and add its IP/DNS name to `-addext`):

```bash
openssl req -x509 -newkey rsa:4096 -nodes -days 365 \
  -keyout kbs/config/docker-compose/tls-key.pem \
  -out kbs/config/docker-compose/tls-cert.pem \
  -subj "/CN=localhost" \
  -addext "subjectAltName=DNS:localhost,IP:127.0.0.1"
```

`kbs/config/docker-compose` is already bind-mounted into the `kbs` container at `/opt/confidential-containers/kbs/user-keys`, so the new files are visible to KBS without touching `docker-compose.yml`.

Edit `kbs/config/docker-compose/kbs-config.toml` and switch the `[http_server]` section to:
...
`tls_profile` defaults to `intermediate` (TLS 1.2 + 1.3, balanced compatibility) and can be omitted. Use `modern` to require TLS 1.3 only, or `custom` with `tls_min_version` / `tls_max_version` / `tls_ciphers` / `tls_groups` for full control, including post-quantum key exchange groups. See the [KBS TLS configuration reference](https://github.com/confidential-containers/trustee/blob/main/kbs/docs/config.md#tls-configuration) for every profile and option.

Recreate the `kbs` container to pick up the config change:

```bash
docker compose up -d --force-recreate kbs
```

Verify the endpoint answers over TLS:

```bash
curl --cacert kbs/config/docker-compose/tls-cert.pem https://localhost:8082/kbs/v0/resource-policy
```

A `401`/`403` JSON error (instead of a connection or certificate error) confirms KBS is terminating TLS — you're reaching the server, just without valid credentials for that endpoint yet.

With TLS enabled, every KBS client needs to trust `tls-cert.pem` (or its issuing CA for a real deployment):

- **`kbs-client`** — pass `--cert-file` and use `https://` in `--url`:

  ```bash
  ./target/release/kbs-client \
    --url https://localhost:8082 \
    --cert-file kbs/config/docker-compose/tls-cert.pem \
    config \
    set-resource \
...
- **Attestation Agent / Proplet** (in-guest, used both for the AA's own attestation flow and for Proplet's `image-rs`-based decryption) — set `cert` under `[token_configs.kbs]` in the AA config to the certificate's **PEM content** (not a file path). See [Configure Proplet for TEE mode](#configure-proplet-for-tee-mode) below.

> The `coco_keyprovider` container used later to encrypt images only _embeds_ the `kbs:///...` resource URI as an annotation — it does not contact KBS over the network during encryption, so no cert is needed for that step even with TLS enabled.
...
This section covers a local QEMU CVM, which needs TEE support on the host you run it from. For a managed cloud CVM — where the hypervisor is not yours and the provider exposes a vTPM rather than TEE device nodes — see [Azure Confidential VMs](/azure-cvm) instead. That guide deploys Trustee separately from the guest stack, and also exists as [`hal/azure/README.md`](https://github.com/absmach/propeller/blob/main/hal/azure/README.md).
...
| `PROPLET_ENTITY_ID`  | Unique client identifier | `proplet-worker-01` |
| `PROPLET_API_KEY`    | Authentication key       | `secret-key-here`   |
...
| `KBS_CERT_PATH`        | Path to a PEM cert to trust for an `https://` `KBS_URL` | none (unset)           |
| `ENABLE_CVM`           | CVM mode (`auto`/`tdx`/`sev`/`snp`/`se`/`cca`/`none`)   | `auto`                 |
| `RAM`                  | VM memory                                               | `16384M`               |
| `CPU`                  | CPU cores                                               | `4`                    |
| `DISK_SIZE`            | Disk size                                               | `40G`                  |

Against a [TLS-enabled KBS](/tee#enable-tls-on-the-kbs-endpoint), set `KBS_URL` to `https://...` and `KBS_CERT_PATH` to the certificate file (e.g. `tls-cert.pem` generated earlier). `qemu.sh` embeds its contents as the `cert` field under `[token_configs.kbs]` in the generated `/etc/attestation-agent.conf`, the same field described in [Configure Proplet for TEE mode](#configure-proplet-for-tee-mode) — no manual edits needed after boot.
...
# For a TLS-enabled KBS instead:
# export KBS_URL="https://10.0.2.2:8082"
# export KBS_CERT_PATH="/path/to/tls-cert.pem"
./qemu.sh
```
...
- `url` — KBS endpoint URL. Use `https://` once KBS has [TLS enabled](/tee#enable-tls-on-the-kbs-endpoint).
- `cert` (optional) — the KBS TLS certificate's **PEM content**, inlined directly in the TOML file with triple-quoted strings (not a file path). Required when `url` is `https://` and the certificate isn't already trusted by the guest's system CA store, e.g. a self-signed cert.
- `tee_key_algorithm` (optional) — key exchange algorithm, defaults to `ECDH-ES+A256KW-P256`

This same file also backs Proplet's `image-rs`-based decryption path in TEE mode (via `PROPLET_AA_CONFIG_PATH`), so `cert` only needs to be set once here — it doesn't need to be configured separately for encryption/decryption.

With TLS enabled on KBS, the config looks like this:

```toml
[token_configs]

[token_configs.kbs]
url = "https://10.0.2.2:8082"
cert = '''
-----BEGIN CERTIFICATE-----
...
Paste the full contents of `tls-cert.pem` (generated in [Enable TLS on the KBS endpoint](/tee#enable-tls-on-the-kbs-endpoint)) between the `'''` markers.
