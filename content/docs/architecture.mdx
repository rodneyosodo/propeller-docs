...
The [Propeller Kubernetes Operator](https://github.com/absmach/propeller-k8s-operator) is a Kubernetes operator that manages and schedules **WebAssembly (WASM) tasks** on a hybrid fleet of workers, which can be either Kubernetes pods or external devices. It introduces two primitives — **Proplet**, the hybrid worker resource (k8s or external), and **Task**, the canonical task API — plus supporting Custom Resources for batching and federated learning:

**Custom Resources:**

- `Proplet`: a worker node. The operator creates and reconciles a Kubernetes `Deployment` for `k8s`-type proplets and monitors `external` devices over MQTT; no K8s resources are created for the latter.
- `Task`: the smallest unit of work, a single WASM workload executed by a proplet. The module comes from an embedded `file` (base64 bytes) or an `imageUrl` — an OCI registry reference resolved through the registry proxy. `imageUrl` is a WASM module reference, never a container image; the operator never runs it as one.
- `PropellerJob`: groups multiple `Task`s into a managed batch with `parallel`, `sequential`, or `configurable` execution modes.
- `FederatedJob` and `TrainingRound`: orchestrate multi-round federated learning experiments layered on top of `Task`, with k-of-n update aggregation.

**Key Behaviors:**

- Tasks always dispatch to the proplet's own Wasmtime runtime via MQTT, regardless of whether the proplet is `k8s` or `external`.
- The operator reconciles proplet Deployments, schedules WASM workloads, and aggregates results through its controllers: `PropletReconciler`, `TaskReconciler`, `PropellerJobReconciler`, `FederatedJobReconciler`, and `TrainingRoundReconciler`.

See the [Kubernetes Operator](/k8s/operator) documentation for the full architecture.
...
m/{tenantID}/c/{channelID}/control/proplet/create
    └─ Discovery announcement on startup

m/{tenantID}/c/{channelID}/control/manager/start
    └─ Task start commands from Manager

m/{tenantID}/c/{channelID}/control/manager/stop
    └─ Task stop commands from Manager

m/{tenantID}/c/{channelID}/registry/server
    └─ Incoming Wasm binary chunks from Registry

m/{tenantID}/c/{channelID}/registry/proplet
    └─ Requests for Wasm binary chunks to Registry

m/{tenantID}/c/{channelID}/control/proplet/results
    └─ Task execution results back to Manager

m/{tenantID}/c/{channelID}/control/proplet/metrics
    └─ Proplet-level resource metrics (CPU, memory, cgroup stats)

m/{tenantID}/c/{channelID}/control/proplet/task_metrics
    └─ Per-task process metrics (CPU, memory, disk I/O, threads, file descriptors)
```
