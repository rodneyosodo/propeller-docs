...
1. Installs system packages (`build-essential`, `libssl-dev`, `protobuf-compiler`, `libtss2-dev`, `tpm2-tools`, `mosquitto-clients`, `rustup`, and others)
2. Installs the Rust toolchain via `rustup`
3. Downloads and installs Wasmtime v44.0.0 from GitHub
4. Clones and compiles the **Attestation Agent** from [guest-components](https://github.com/rodneyosodo/guest-components) (`upstream-proplet` branch) with all attesters enabled
...
7. Verifies all four binaries (`wasmtime`, `attestation-agent`, `coco_keyprovider`, `proplet`) and the OCI decrypt config exist before enabling any services
...
```bash
export PROPLET_TENANT_ID="a93fa93e-30d0-425e-b5d1-c93cd916dca7"
export PROPLET_ENTITY_ID="c902e51c-5eac-4a2d-a489-660b5f7ab461"
export PROPLET_API_KEY="75a0fefe-9713-478d-aafd-72032c2d9958"
...
The Attestation Agent config at `/etc/attestation-agent.conf`:
...
export PROPLET_ENTITY_ID="client-1"
# ... set other vars
(cd vm1 && ./qemu.sh build)

export PROPLET_ENTITY_ID="client-2"
# ... set other vars
...
The script above targets local QEMU and needs TEE support on the host you run it from. For a managed cloud CVM — where the hypervisor is not yours and the provider exposes a vTPM rather than TEE device nodes — see [Azure Confidential VMs](/azure-cvm). That guide deploys Trustee separately from the guest stack and covers the Azure-specific attestation path.
