...
Place your CA certificate at `docker/ca.crt` (or adjust the source path in the volume mount).
...
| HTTP Proxy  | `wasmtime serve -Shttp <file> [cli_args…]` | HTTP server for WASI `incoming-handler` |
...
The task must set `"daemon": true` since `wasmtime serve` runs indefinitely. When `daemon` is true, the proplet tracks the subprocess and reports its PID rather than waiting for it to exit.
