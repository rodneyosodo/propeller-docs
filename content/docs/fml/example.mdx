...
If you have **mosquitto** or another MQTT broker running as a system service, it will conflict with Atom's MQTT adapter. Stop it before proceeding:
...
The FL demo extends the base Atom + Propeller stack with FL-specific services. You must use both compose files together.
...
```
service "local-data-store" refers to undefined network propeller-base-net
```
...
[+] Running 36/36
 ...
 ✔ Container propeller-nats             Started                              0.8s
 ✔ Container propeller-rabbitmq         Started                              0.9s
 ✔ Container propeller-spicedb-db       Started                              0.7s
 ✔ Container propeller-auth-db          Started                              0.8s
 ✔ Container propeller-users-db         Started                              0.7s
 ✔ Container propeller-clients-db       Started                              0.8s
 ✔ Container propeller-domains-db       Started                              0.9s
 ✔ Container propeller-channels-db      Started                              0.8s
 ✔ Container propeller-spicedb          Started                              1.2s
 ✔ Container propeller-spicedb-migrate  Started                              1.5s
 ✔ Container propeller-auth             Started                              2.1s
 ✔ Container propeller-users            Started                              2.3s
 ✔ Container propeller-domains          Started                              2.4s
 ✔ Container propeller-clients          Started                              2.5s
 ✔ Container propeller-channels         Started                              2.6s
 ✔ Container propeller-nginx            Started                              2.8s
 ✔ Container propeller-mqtt             Started                              2.9s
...
- **Atom stack**: Auth, Tenants, Entities, Channels, MQTT Adapter, Nginx, SpiceDB
...
propeller-auth               Up 10 seconds   0.0.0.0:8189->8189/tcp
propeller-auth-db            Up 10 seconds   5432/tcp
propeller-channels           Up 10 seconds   0.0.0.0:9005->9005/tcp
propeller-channels-db        Up 10 seconds   5432/tcp
propeller-clients            Up 10 seconds   0.0.0.0:9006->9006/tcp
propeller-clients-db         Up 10 seconds   5432/tcp
propeller-domains            Up 10 seconds   0.0.0.0:9003->9003/tcp
propeller-domains-db         Up 10 seconds   5432/tcp
propeller-mqtt               Up 10 seconds   0.0.0.0:1883->1883/tcp
propeller-nats               Up 10 seconds   4222/tcp, 6222/tcp, 8222/tcp
propeller-nginx              Up 10 seconds   0.0.0.0:80->80/tcp, 0.0.0.0:443->443/tcp
propeller-rabbitmq           Up 10 seconds   5672/tcp, 15672/tcp
propeller-spicedb            Up 10 seconds   50051/tcp
propeller-spicedb-db         Up 10 seconds   5432/tcp
propeller-spicedb-migrate    Up 10 seconds
propeller-users              Up 10 seconds   0.0.0.0:9002->9002/tcp
propeller-users-db           Up 10 seconds   5432/tcp
```
...
## Step 4: Provision Atom Resources

Before the manager and proplets can connect to MQTT, you must provision the necessary Atom resources (tenant, channel, and entities).

Provisioning creates persistent data in Atom databases. Only needs to be repeated if you remove Docker volumes (e.g., `docker compose down -v`).
...
```
============================================================
Atom Provisioning Script for FL Demo
============================================================
...
The **proplet-1**, **proplet-2**, and **proplet-3** entity IDs shown in the summary are Atom-generated UUIDs — created when the script registers each proplet as an entity in your Atom tenant. These are what you pass as `participants` when triggering FL experiments. The script automatically writes them to `docker/.env`.

### Verify Environment Variables Were Set

```bash
grep -E "PROPLET.*ENTITY_ID|MANAGER_TENANT_ID|MANAGER_CHANNEL_ID" docker/.env | grep -v '=$'
```

Your output should look like this:

```
MANAGER_TENANT_ID=4e484e5b-0498-4453-adf8-a6fa6a37454b
MANAGER_CHANNEL_ID=c1578c6c-598e-4dbe-9a8b-5d2acd623679
PROPLET_ENTITY_ID=bf7e3581-4dac-472c-bcd9-a83a6ca53197
PROPLET_2_ENTITY_ID=cf3738c1-07db-41c7-8fc3-234202cf3642
PROPLET_3_ENTITY_ID=095d2d54-4017-4da1-b589-0809d64aa517
```
...
```
[+] Running 13/13
 ✔ Container propeller-clients-db   Running                              0.0s
 ✔ Container propeller-users-db     Running                              0.0s
 ✔ Container propeller-nats         Running                              0.0s
 ...
...
```bash
PROPLET_ENTITY_ID=$(grep '^PROPLET_ENTITY_ID=' docker/.env | grep -v '=""' | tail -1 | cut -d '=' -f2 | tr -d '"')
curl -s "http://localhost:8083/datasets/$PROPLET_ENTITY_ID" | jq '{schema: .schema, proplet_id: .proplet_id, size: .size}'
```
...
```bash
curl -s "http://localhost:8083/datasets/$PROPLET_ENTITY_ID" | jq '.data[0:3]'
```
