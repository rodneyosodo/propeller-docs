...
- **Powerful Service Mesh**: Integrates with [**Atom**](https://github.com/absmach/atom) and [**FluxMQ**](https://github.com/absmach/fluxmq) for secure, efficient IoT device communication and authorization.
...
- **Job Orchestration**: Group multiple tasks into jobs with configurable execution modes (parallel, sequential, or configurable), enabling complex multi-step workflows.
- **Process Monitoring**: Collect per-task OS-level metrics (CPU, memory, disk I/O, thread count, file descriptors) from every running WebAssembly workload and publish them over MQTT for real-time observability.
