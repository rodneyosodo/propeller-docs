...
The Manager communicates with proplets over MQTT using the [Magistrala](https://github.com/absmach/magistrala) message broker. Magistrala provides secure, multi-tenant messaging with built-in authentication and authorization—ensuring that only authorized components can publish and subscribe to Propeller's control channels.
...
| `MANAGER_DOMAIN_ID`    | Magistrala domain identifier          | From `config.toml`     |
| `MANAGER_CHANNEL_ID`   | Magistrala channel identifier         | From `config.toml`     |
...
The `MANAGER_DOMAIN_ID`, `MANAGER_CHANNEL_ID`, `MANAGER_CLIENT_ID`, and `MANAGER_CLIENT_KEY` values come from Magistrala provisioning. The `propeller-cli provision` command automates this setup by:

1. Authenticating with your Magistrala instance
...
The wizard prompts for Magistrala credentials and generates a `config.toml` file:
...
| Field        | Purpose                                          |
| ------------ | ------------------------------------------------ |
| `domain_id`  | Magistrala domain for multi-tenant isolation     |
...
```
m/{domain_id}/c/{channel_id}/control/proplet/results
```
