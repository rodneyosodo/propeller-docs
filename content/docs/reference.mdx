...
| `--atom-url`         | `-a`  | `http://localhost:8080` | Atom base service URL               |
...
| `file`               | string          | Base64-encoded Wasm binary for direct embedding (redacted with `<REDACTED>` in API responses to avoid leaking binary contents). |
...
| `broadcast`          | boolean         | If `true`, send the task to all available proplets simultaneously instead of a single selected one.                             |
| `monitoring_profile` | object          | Per-task monitoring configuration (see [Monitoring](monitoring)).                                                               |
| `metadata`           | object          | Free-form JSON key-value metadata attached to the task (max 1 MB serialized). The reserved `elastic` sub-map holds `wasi_security` / `wasi_pep` and is the only part forwarded to the proplet — see [WASI Security Policies](/wasi-security).                                                   |
