...
When running via proplet with `PROPLET_HTTP_TLS_CA_CERT` set, outgoing requests from WASM components can also connect to internal HTTPS servers using custom or self-signed certificates — see [Proplet: HTTP TLS](/proplet#http-tls).

## Source Code

The source code is available in the [examples/http-client](https://github.com/absmach/propeller/blob/main/examples/http-client/src/main.rs) directory.
